 ...
 */
// wwwroot/assets/js/dependencies/app.js
// تب‌ها
initializeTabs();
// مودال فیلتر شهر
...
window.initializeApp = function () {
    // آف‌کنوس منو و سبد خرید
    initializeOffcanvas();
    // هر چیز دیگه‌ای که داری (اسلایدر، tooltip و ...)
};

// تابع تب‌ها (همون کدی که قبلاً دادم)
function initializeTabs() {
    ...
    // تب اول رو فعال کن
    document.querySelector('.tab-button[data-tab="Intro"]')?.click();
}

// مودال فیلتر
function initializeFilterModal() {
    const triggers = document.querySelectorAll('.modal-trigger[data-modal-target="filterModal"]');
    const modal = document.getElementById('filterModal');
    const closeBtns = modal?.querySelectorAll('[data-modal-close]');

    triggers.forEach(t => t.onclick = () => modal.classList.remove('hidden'));
    closeBtns?.forEach(b => b.onclick = () => modal.classList.add('hidden'));
}

// آف‌کنوس
window.toggleOffcanvas = function (id) {
    document.getElementById(id).classList.add('visible', 'translate-x-0', 'opacity-100');
...
window.closeOffcanvas = function () {
    document.querySelectorAll('.offcanvas').forEach(o => {
        ...
        o.classList.add('invisible', '-translate-x-full');
    });
...
if (tabs.length > 0 && tabContents.length > 0) {
    tabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            tabs.forEach(t => {
                t.classList.remove('active', 'border-primary', 'text-green-600', 'dark:border-primary-dark', 'dark:text-primary-dark');
            ...
            tab.classList.add('active', 'border-primary', 'text-green-600', 'dark:border-primary-dark', 'dark:text-primary-dark');
...
/**
 * toggle code adding in version 3
 */
