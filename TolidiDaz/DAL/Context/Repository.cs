...
namespace DAL.Context
{
    public class Repository<TEntity> : IRepository<TEntity> where TEntity : Base
    {
        ...
        public virtual async Task<List<int>> AddRangeAsync(List<TEntity> entities)
        {

            await _dbSet.AddRangeAsync(entities);
            var res = await _unitOfWork.SaveChangesAsync();
            return res > 0 ? entities.Select(s=>s.ID).ToList() : [];
        }
        ...
        {
            //  var entry = _unitOfWork.Entry(entityToUpdate);
            var attachedEntity =await _dbSet.FindAsync(entityToUpdate.ID);
            if (attachedEntity != null)
            {
                var attachedEntry = _unitOfWork.Entry(attachedEntity);
                attachedEntry.CurrentValues.SetValues(entityToUpdate);
        ...
        public virtual async Task<IEnumerable<TEntity>> GetAllAsync<TOrderKey, TInclude>(Expression<Func<TEntity, TOrderKey>> orderBy, Expression<Func<TEntity, bool>> predicate, Expression<Func<TEntity, TInclude>> include, int Page, int take)
        {
            ...
            return await query
                .Skip(Skip)
                .Take(take).AsNoTracking().AsNoTracking().ToListAsync();
        }
        ...
        {
            return await _dbSet.AsNoTracking().OrderBy(p => p.ID).LastOrDefaultAsync();
        }
        ...
        public virtual async Task<IEnumerable<TResult>> GetAllAsync<TResult>( Expression<Func<TEntity, TResult>> selector, int page, int take)
        {
            IQueryable<TEntity> query = _dbSet;
            int Skip = (take * page) - take;
            return await query
        ...
        public virtual async Task<IEnumerable<TResult>> GetAllAsync<TResult>(Expression<Func<TEntity, bool>> predicate, Expression<Func<TEntity, TResult>> selector, int page, int take)
        {
            IQueryable<TEntity> query = _dbSet.Where(predicate);
            int Skip = (take * page) - take;
            return await query
        ...
        public virtual async Task<TValue?> GetMaxAsync<TValue>(Expression<Func<TEntity, TValue>> ScalerFunction)
      where TValue : struct, IComparable
        {
            if (!_dbSet.Any())
                return null;

            return await _dbSet.MaxAsync(ScalerFunction);
        }
        public virtual async Task<TValue?> GetMaxAsync<TValue>(
        ...
        public virtual async Task<IEnumerable<TEntity>> GetTopAsync<TOrderKey>
        (Expression<Func<TEntity, TOrderKey>> orderByDesc,int take)
        {
            return await _dbSet
        ...
        public virtual async Task<IEnumerable<TEntity>> GetTopAsync<TOrderKey>
       (Expression<Func<TEntity, TOrderKey>> orderByDesc, Expression<Func<TEntity, bool>> predicate, int take)
        {
            return await _dbSet
                .OrderByDescending(orderByDesc)
                .Take(take)
